...
for episode in range(num_episodes):
    ...
    if (episode + 1) % 5000 == 0:
        avg_reward = np.mean(rewards_all_episodes[-5000:])
        max_reward = np.max(rewards_all_episodes[-5000:])
        print(f"Episodio: {episode+1}/{num_episodes}, Avg: {avg_reward:.2f}, Max: {max_reward:.1f}, Eps: {agent.epsilon:.3f}")
